...
export async function fetchJobs(params: {
  ...
  search.set("sort", params.sort || "best");

  const r = await fetch(`${API_BASE}/v80/jobs?${search.toString()}`);
  if (!r.ok) {
    throw new Error(`Failed to load jobs from Supabase: ${r.status}`);
  }
  return r.json();
}

export const fetchV78Jobs = fetchJobs;
...
export async function fetchStats(
  ...
  const r = await fetch(`${API_BASE}/v80/stats?${search.toString()}`);
  if (!r.ok) {
    throw new Error(`Failed to load Supabase stats: ${r.status}`);
  }
  return r.json();
}
...
export async function updateJobStatus(jobId: number, status: string) {
  ...
  if (!r.ok) {
    throw new Error(`Failed to update job status: ${r.status}`);
  }
  return r.json();
}
