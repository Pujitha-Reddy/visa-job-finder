...
def _load_rollup(conn):
    rows = conn.execute("""
        SELECT normalized_name, sponsor_strength, total_filings, recent_filings
        FROM sponsor_rollup
    """).fetchall()
    return {
        r["normalized_name"]: {
...
def enrich_all_jobs_v11():
    ...
    with _conn() as conn:
        ...
        for job in jobs:
            ...
            match_name = base if base in history else None
            method = "EXACT"

            if not match_name:
                match_name, confidence, method = best_unique_match(entity, sponsor_names)
